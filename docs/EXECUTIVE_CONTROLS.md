...
- **Exclusão de conta**: a ação encerra identidades e sessões no Auth, bloqueia imediatamente as permissões do painel e retira a conta da lista de gestão. O registro técnico do usuário permanece para preservar os vínculos de vendas, abordagens, saques e CRM; o evento final é auditado quando o Auth confirma a exclusão lógica. Não é necessário suspender a conta manualmente antes de excluir.
- **Último login**: valor original de `auth.users.last_sign_in_at`. Exibição com segundos em `America/Sao_Paulo`, original UTC disponível no detalhe. Não representa presença on-line ou abertura da página. Consulta a cada 50 segundos, no foco e por eventos; indisponibilidade é indicada explicitamente.
