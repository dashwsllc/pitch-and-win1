        ...
        <DialogContent className={`max-h-[92vh] overflow-y-auto sm:max-w-2xl ${compact ? 'gap-3 p-4' : ''}`} data-lenis-prevent>
          ...
          {form && editing && <form onSubmit={submit} className={compact ? 'space-y-4' : 'space-y-6'}>
            ...
            <fieldset disabled={saving} className="space-y-4 border-t border-border pt-4"><legend className="px-1 text-xs font-medium uppercase tracking-wider text-electric">Permissões e comissão</legend>
              ...
              <p className="text-[10px] text-muted-foreground">SDR acessa somente a operação SDR; Closer, somente a operação Closer; Executive opera as duas áreas sem acesso administrativo. Apenas Super Admin gerencia contas e permissões. A opção Acesso ao CRM concede leitura do CRM a outros cargos, sem ampliar suas ações operacionais. Para bloquear uma conta, use Suspender acesso.</p>
            </fieldset>
