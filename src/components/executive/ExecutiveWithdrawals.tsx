...
export function ExecutiveWithdrawals() {
  ...
  const query = useQuery({ queryKey:['executive-withdrawals',user?.id,page], enabled:!!user,
    queryFn:async () => {
  ...
  return <section className="surface-panel rounded-2xl p-5 sm:p-6"><h2 className="flex items-center gap-2 text-lg font-medium text-white"><Wallet className="h-5 w-5 text-electric" />Saques em revisão</h2><p className="mt-2 text-xs leading-relaxed text-muted-foreground">Confira valores reservados antes de excluir vendas. Uma solicitação incorreta pode ser rejeitada com motivo, liberando a reserva. Pagamentos já realizados permanecem preservados.</p>
    ...
    <Dialog open={!!selected} onOpenChange={open => { if(!open && !saving) setSelected(null) }}><DialogContent><DialogHeader><DialogTitle>Rejeitar solicitação de saque?</DialogTitle><DialogDescription>A reserva será liberada. O Seller poderá consultar o motivo. Nenhuma transferência bancária é executada por esta ação.</DialogDescription></DialogHeader><div className="space-y-2"><Label htmlFor="withdrawal-reason">Motivo obrigatório</Label><Textarea id="withdrawal-reason" maxLength={2000} value={reason} onChange={e => setReason(e.target.value)} /></div><DialogFooter><Button variant="outline" disabled={saving} onClick={() => setSelected(null)}>Cancelar</Button><Button variant="destructive" disabled={saving || reason.trim().length<5} onClick={regularize}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Rejeitar e liberar reserva</Button></DialogFooter></DialogContent></Dialog>
  </section>
}
