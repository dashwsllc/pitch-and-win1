    ...
    <DialogContent className="max-h-[92dvh] overflow-y-auto sm:max-w-xl" data-lenis-prevent>
      ...
      <form onSubmit={save} className="space-y-4">
        <fieldset disabled={busy} className="space-y-4">
          ...
          <DialogFooter><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="submit" disabled={financialChanged && (catalog.isPending || catalog.isError)}>{busy && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Salvar alterações</Button></DialogFooter>
        </fieldset>
      </form>
    </DialogContent>
  </Dialog>
