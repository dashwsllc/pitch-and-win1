...
export const ATHLETE_POSITIONS = ['Goleiro', 'Zagueiro', 'Lateral', 'Volante', 'Meia', 'Atacante']
...
export function contactFromLead(lead: CRMLead): ContactForm {
  return Object.fromEntries(
    Object.keys(emptyContact).map((key) => [key, String(lead[key as keyof ContactForm] ?? '')])
  ) as ContactForm
}
export function contactPayload(form: ContactForm) {
  return {
    ...
    athlete_height_cm: form.athlete_height_cm === '' ? null : Number(form.athlete_height_cm),
    athlete_weight_kg: form.athlete_weight_kg === '' ? null : Number(form.athlete_weight_kg),
...
export function validateContact(form: ContactForm) {
  ...
  if (form.phone.trim().length < 8 || form.phone.trim().length > 32) return 'Informe um WhatsApp válido.'
  if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(form.email.trim())) return 'Informe um e-mail válido.'
  ...
  if (!ATHLETE_POSITIONS.includes(form.athlete_position)) return 'Selecione a posição em campo.'
  if (
  ...
  if (form.performance_report_url && !/^https:\/\/[^/\s]+/.test(form.performance_report_url))
    return 'Use um link HTTPS para o relatório.'
  return null
}

export const callDate = (value: string | null) =>
  value ? new Date(value).toLocaleString('pt-BR', { dateStyle: 'short', timeStyle: 'short' }) : 'Sem horário'
