...
const name = 'arena_role_cycle_sync'
...
const verification = readFileSync(resolve('scripts/verify-arena-role-cycle-sync.sql'), 'utf8')
const tag = `$arena_role_cycle_${version}$`
...
writeFileSync(resolve('.verification.local/arena-role-cycle-check.sql'), query)
...
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy',
  '--file', '.verification.local/arena-role-cycle-check.sql', '--output-format', 'json')
