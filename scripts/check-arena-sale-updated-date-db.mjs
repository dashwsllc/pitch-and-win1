...
const version = '20260923170000'
const name = 'arena_sale_updated_date'
...
const verification = readFileSync(resolve('scripts/verify-arena-sale-updated-date.sql'), 'utf8')
const tag = `$arena_sale_date_${version}$`
...
writeFileSync(resolve('.verification.local/arena-sale-updated-date.sql'), query)
const cli = process.env.SUPABASE_CLI || 'npx'
const args = process.env.SUPABASE_CLI ? [] : ['--yes', 'supabase']
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy', '--file', '.verification.local/arena-sale-updated-date.sql', '--output-format', 'json')
