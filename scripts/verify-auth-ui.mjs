...
try {
  ...
  await expect(page.getByLabel('Cargo', { exact: true })).toHaveCount(0)
  ...
  assert(fields.every(f => f.text === 'rgb(240, 239, 241)'))
  ...
  assert.equal(signupPayload?.data?.requested_role, undefined)
  ...
  console.log('PASS: desktop/mobile, no public role field, AA contrast, validation, spinner and access request without session')
