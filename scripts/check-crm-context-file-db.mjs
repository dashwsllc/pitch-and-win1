...
const version = '20260912220000'
const name = 'crm_context_markdown'
...
const tests = readFileSync(resolve(root, 'supabase/tests/crm_context_markdown.sql'), 'utf8')
...
SELECT '${apply ? 'CRM Markdown migration applied' : 'CRM Markdown assertions passed; fixtures rolled back'}' AS result;`
mkdirSync(resolve(root, '.verification.local'), { recursive: true })
writeFileSync(resolve(root, '.verification.local/crm-context-file-query.sql'), query)
const result = spawnSync('npx', ['supabase', 'db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy', '--file', '.verification.local/crm-context-file-query.sql', '--output', 'json'], {
