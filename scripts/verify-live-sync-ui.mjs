...
try {
  ...
  assert.equal(await page.evaluate(() => window.__dashboardRefreshEvents), 0, 'Focus and the old 50-second interval must not refresh the monitor')
  await expect(page.locator('[data-dashboard-section="commercial-indicators"] .animate-pulse')).toHaveCount(0)
  await page.evaluate(() => window.dispatchEvent(new Event('dashboard-data-changed')))
  ...
  console.log('PASS: the Brasília clock advances, monitor indicators stay mounted through 50 seconds and background data changes never show loading skeletons.')
} finally {
  await context.close()
  await browser.close()
}
