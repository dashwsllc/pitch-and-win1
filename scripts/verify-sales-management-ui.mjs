...
try {
  ...
  await expect(page.getByRole('button',{name:'Ver vendas registradas e corrigir data da compra'})).toBeVisible()
  await page.getByRole('button',{name:'Ver vendas registradas e corrigir data da compra'}).click()
  await expect(board.getByRole('article')).toHaveCount(2)
  await page.screenshot({path:'.verification.local/sales-super-admin-date-action.png',fullPage:true})
  const approvedCard = board.getByRole('article',{name:'Venda de Comprador 1'})
  await approvedCard.getByRole('button',{name:'Corrigir data da compra de Comprador 1'}).click()
  ...
  await pendingCard.getByRole('button',{name:'Corrigir data da compra de Comprador 2'}).click()
  ...
  await page.goto(`${origin}/vendas`)
  await expect(board.getByRole('article')).toHaveCount(2)
  await expect(page.getByRole('button',{name:'Ver vendas registradas e corrigir data da compra'})).toHaveCount(0)
  await expect(board.getByRole('button',{name:/Corrigir data da compra/})).toHaveCount(0)
