...
const version = '20260923210000'
const name = 'arena_goals_ranking_alignment'
...
const verification = readFileSync(resolve('scripts/verify-goals-ranking.sql'), 'utf8')
const tag = `$goal_ranking_${version}$`
...
writeFileSync(resolve('.verification.local/goals-ranking.sql'), query)
...
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy',
  '--file', '.verification.local/goals-ranking.sql', '--output-format', 'json')
