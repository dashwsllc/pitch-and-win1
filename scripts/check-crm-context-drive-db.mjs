...
const version = '20260912233000'
const name = 'crm_context_drive_media'
...
const tests = readFileSync(resolve(root, 'supabase/tests/crm_context_drive_media.sql'), 'utf8')
...
SELECT '${apply ? 'CRM Drive media migration applied' : 'CRM Drive media assertions passed; fixtures rolled back'}' AS result;`
mkdirSync(resolve(root, '.verification.local'), { recursive: true })
writeFileSync(resolve(root, '.verification.local/crm-context-drive-query.sql'), query)
const result = spawnSync('npx', ['supabase', 'db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy', '--file', '.verification.local/crm-context-drive-query.sql', '--output', 'json'], {
