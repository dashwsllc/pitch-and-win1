...
const version = '20260923200000'
const name = 'arena_cancellation_totals'
...
const verification = readFileSync(resolve('scripts/verify-arena-cancellations.sql'), 'utf8')
const tag = `$arena_cancellation_${version}$`
...
writeFileSync(resolve('.verification.local/arena-cancellations.sql'), query)
const cli = process.env.SUPABASE_CLI || 'npx'
const args = process.env.SUPABASE_CLI ? [] : ['--yes', 'supabase']
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy', '--file', '.verification.local/arena-cancellations.sql', '--output-format', 'json')
