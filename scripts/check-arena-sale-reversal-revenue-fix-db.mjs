...
const version = '20260925220000'
const name = 'arena_sale_reversal_revenue_fix'
...
const verification = readFileSync(resolve('scripts/verify-arena-sale-reversal-revenue-fix.sql'), 'utf8')
const tag = `$arena_sale_reversal_revenue_fix_${version}$`
...
writeFileSync(resolve('.verification.local/arena-sale-reversal-revenue-fix-check.sql'), query)
...
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy',
  '--file', '.verification.local/arena-sale-reversal-revenue-fix-check.sql', '--output-format', 'json')
