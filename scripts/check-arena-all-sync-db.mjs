...
const version = '20260923180000'
const name = 'arena_sync_all_roles'
...
const verification = readFileSync(resolve('scripts/verify-arena-all-sync.sql'), 'utf8')
const tag = `$arena_sync_${version}$`
...
writeFileSync(resolve('.verification.local/arena-all-sync.sql'), query)
const cli = process.env.SUPABASE_CLI || 'npx'
const args = process.env.SUPABASE_CLI ? [] : ['--yes', 'supabase']
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy', '--file', '.verification.local/arena-all-sync.sql', '--output-format', 'json')
