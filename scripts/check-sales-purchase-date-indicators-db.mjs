...
const name = 'sales_purchase_date_indicators'
...
const verification = readFileSync(resolve('scripts/verify-sales-purchase-date-indicators.sql'), 'utf8')
const tag = `$sales_purchase_date_${version}$`
...
writeFileSync(resolve('.verification.local/sales-purchase-date-indicators.sql'), query)
const cli = process.env.SUPABASE_CLI || 'npx'
const args = process.env.SUPABASE_CLI ? [] : ['--yes', 'supabase']
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy', '--file', '.verification.local/sales-purchase-date-indicators.sql', '--output-format', 'json')
