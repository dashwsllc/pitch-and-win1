...
await realFixtures(async ({ clients, sessions, users, anon, name, productId, ticketId }) => {
  ...
  try {
    ...
    await expect(secondCard.getByText('Call ainda não agendada', { exact: true })).toBeVisible()
    ...
    await expect(dialog.getByLabel('Responsável', { exact: true })).toBeEnabled()
    await expect(dialog.getByLabel('Responsável', { exact: true })).toHaveValue(actor('second'))
