...
const version = '20260925210000'
const name = 'arena_score_weights_audit_fix'
...
const verification = readFileSync(resolve('scripts/verify-arena-score-weights-audit-fix.sql'), 'utf8')
const tag = `$arena_score_weights_audit_fix_${version}$`
...
writeFileSync(resolve('.verification.local/arena-score-weights-audit-fix-check.sql'), query)
...
args.push('db', 'query', '--linked', '--project-ref', 'mbzwchnxtskysqplqiyy',
  '--file', '.verification.local/arena-score-weights-audit-fix-check.sql', '--output-format', 'json')
