...
try {
  ...
  const saved = must(await admin.from('crm_lead_contexts')
    .select('content,file_name,file_content,file_mime_type').eq('lead_id', leadId).single())
  ...
  console.log('PASS: production browser TXT upload → atomic Markdown storage → byte-identical database/download; non-TXT rejected without partial row')
} finally {
